...
class Siswa extends CI_Controller {
	...
	public function ubah($NoInduk){
		if($this->input->post('submit')){ // Jika user mengklik tombol submit yang ada di form
			if($this->SiswaModel->validation("update")){ // Jika validasi sukses atau hasil validasi adalah TRUE
				$this->SiswaModel->edit($NoInduk); // Panggil fungsi edit() yang ada di SiswaModel.php
				redirect('siswa');
			}
		}

		$data['siswa'] = $this->SiswaModel->view_by($NoInduk);
		$this->load->view('siswa/form_ubah', $data);
	}

	public function hapus($NoInduk){
		$this->SiswaModel->delete($NoInduk); // Panggil fungsi delete() yang ada di SiswaModel.php
		redirect('siswa');
	}
}
